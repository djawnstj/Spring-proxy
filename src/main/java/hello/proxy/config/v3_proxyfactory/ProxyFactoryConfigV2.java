...
public class ProxyFactoryConfigV2 {
    ...
    public OrderControllerV2 orderControllerV2(final LogTrace logTrace) {
        final OrderControllerV2 orderContoller = new OrderControllerV2(orderServiceV2(logTrace));

        final ProxyFactory factory = new ProxyFactory(orderContoller);
        factory.addAdvisor(getAdvisor(logTrace));
        final OrderControllerV2 proxy = (OrderControllerV2) factory.getProxy();
        log.info("ProxyFactory proxy={}, target={}", proxy.getClass(), orderContoller.getClass());
